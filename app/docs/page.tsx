...
export default function DocsPage() {
  return (
    <div className="bg-background min-h-svh">
      <header className="border-border bg-background-elevated flex h-12 items-center gap-3 border-b px-6">
        <Link href="/" className="text-mono text-[11px] tracking-[0.12em] uppercase">
          ← Blacksite UI
        </Link>
        <span className="text-mono text-foreground-muted text-[11px] tracking-[0.1em] uppercase">
          Docs
        </span>
      </header>
      ...
      <main className="mx-auto max-w-3xl space-y-6 p-6">
        ...
        <Panel title="01" subtitle="Bootstrap a Next.js + shadcn app" density="compact">
          <pre className="text-mono bg-background-overlay border-border-strong overflow-x-auto rounded-sm border p-3 text-[12px] leading-relaxed">
        ...
        <Panel title="04" subtitle="Import" density="compact">
          <pre className="text-mono bg-background-overlay border-border-strong overflow-x-auto rounded-sm border p-3 text-[12px] leading-relaxed">
            <span className="text-info">import</span> {`{ StatCard }`}{" "}
        ...
        <h2 className="pt-4 text-xl font-semibold tracking-tight">Conventions</h2>
        <ul className="text-foreground-muted list-disc space-y-1.5 pl-5 text-sm">
          <li>
            All components live under <code className="text-mono">@/components/...</code> after
            install (you can re-alias in <code className="text-mono">components.json</code>).
          </li>
          <li>
          ...
          <li>Charts and maps are pure React; no global state or theme provider required.</li>
        </ul>

        <h2 className="pt-4 text-xl font-semibold tracking-tight">Keyboard</h2>
